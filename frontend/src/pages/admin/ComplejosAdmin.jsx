...
export default function ComplejosAdmin() {
  ...
  const cargar = async () => {
    ...
    try {
      ...
      setDuenos(data.filter((u) => u.rol === 'DuenoComplejo'))
      setComplejos(
        data.flatMap((u) =>
          (u.complejos || []).map((c) => ({
            ...c,
            nombreDueno: u.nombre,
            idDueno: u.idUsuario,
          }))
        )
      )
    ...
    try {
      if (form.modo === 'crear') {
        await api.post('/api/Complejo/CrearComplejo', {
          ...
          idDueno: Number(form.idDueno),
        })
        mostrarToast('Complejo creado correctamente')
      } else {
        await api.put(`/api/Complejo/ActualizarComplejo/${form.idComplejo}`, {
          nombre: form.nombre,
          direccion: form.direccion,
          telefono: form.telefono || null,
  ...
  const handleBorrar = async () => {
    setBorrando(true)
    try {
      await api.delete(`/api/Complejo/EliminarComplejo/${aBorrar.idComplejo}`)
      setABorrar(null)
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Creá, editá y eliminá complejos por dueño</p>
        </div>
        <button
          onClick={() =>
            setForm({
      ...
      {duenos.length === 0 && (
        <p className="bg-amber-50 border border-amber-200 text-amber-700 text-sm rounded-lg px-4 py-3 mb-4">
          No hay usuarios con rol &quot;Dueño de complejo&quot;. Creá uno en la pestaña Usuarios.
        </p>
      )}

      {loading ? (
        <p className="text-gray-500 text-lg py-10 text-center">Cargando complejos...</p>
      ) : error ? (
          ...
          <table className="w-full text-sm min-w-[720px]">
            <thead>
              <tr className="text-left text-gray-500 border-b border-gray-100">
                ...
                <th className="px-5 py-3 font-medium">Canchas</th>
            ...
            <tbody>
              {complejos.length === 0 ? (
                <tr>
                  <td colSpan="6" className="px-5 py-8 text-center text-gray-400">No hay complejos.</td>
                </tr>
              ) : (
                complejos.map((c) => (
                  <tr key={c.idComplejo} className="border-b border-gray-50 hover:bg-gray-50/70 transition-colors">
                    <td className="px-5 py-3 font-medium text-gray-800">{c.nombre}</td>
                    <td className="px-5 py-3 text-gray-500">{c.nombreDueno}</td>
                    <td className="px-5 py-3 text-gray-500">{c.direccion}</td>
                    <td className="px-5 py-3 text-gray-500">{(c.canchas || []).length}</td>
      ...
      {/* Modal crear/editar */}
      <Modal open={!!form} onClose={() => setForm(null)} title={form?.modo === 'crear' ? 'Nuevo complejo' : 'Editar complejo'}>
        {form && (
          <form onSubmit={handleGuardar} className="space-y-4">
            ...
            {form.modo === 'crear' ? (
              <div>
                ...
                >
                  {duenos.map((d) => (
                    <option key={d.idUsuario} value={d.idUsuario}>{d.nombre} ({d.email})</option>
                  ))}
                </select>
              </div>
            ) : (
      ...
      <ConfirmDialog
        ...
        message={`¿Seguro que querés eliminar ${aBorrar?.nombre ?? ''}? Se eliminarán también sus canchas y turnos.`}
